...
/**
 * Fills a brand-new, empty site with sample stories so the front page is
 * never blank. Skips straight away if anything has already been published —
 * including everything seeded here, since deleting a placeholder should not
 * bring it back on the next request.
 */
async function seedIfEmpty() {
  const existing = await one('SELECT id FROM articles LIMIT 1');
  if (existing) return;

  for (const sample of [...SAMPLE_ARTICLES, ...GUIDE_ARTICLES]) {
    await query(
      `INSERT INTO articles
        (slug, title, excerpt, content, category, tags, author, featured, cover_image, cover_credit)
       VALUES ($1,$2,$3,$4,$5,$6,'SHAM',$7,$8,$9)
...
export async function ensureSchema() {
  if (!globalForPool.__shamSchema) {
    globalForPool.__shamSchema = (async () => {
      ...
      // Best-effort: a seeding hiccup should never stop the site from
      // working, so it is logged rather than allowed to fail the schema.
      await seedIfEmpty().catch((err) => {
        console.error('[sham] sample-article seeding skipped:', err.message);
      });
    })().catch((err) => {
...
/**
 * Same as tryDb, but keeps the actual error message instead of swallowing
 * it. Only for admin-only screens — the message can include the database
 * host or username, which is fine behind a password but not for a public
 * page.
 */
export async function tryDbDetailed(factory) {
  try {
    return { data: await factory(), error: null };
  } catch (err) {
    console.error('[sham] database unavailable:', err.message);
