...
const GUIDE_SLUGS = ['welcome-to-sham', 'how-the-sections-work', 'sharing-and-search'];

await test('an empty database seeds itself with placeholder stories', async () => {
  await db.ensureSchema();
  const seeded = await db.listArticles({ limit: 50 });
  assert.ok(seeded.length >= 10, 'expected at least 10 placeholder stories, got ' + seeded.length);
  assert.ok(seeded.some((a) => a.featured), 'expected one placeholder to be featured');
  const newsPlaceholders = seeded.filter((a) => !GUIDE_SLUGS.includes(a.slug));
  assert.ok(newsPlaceholders.length >= 8);
  assert.ok(
    newsPlaceholders.every((a) => a.excerpt.toLowerCase().includes('placeholder')),
    'every seeded news story should say "placeholder" in its excerpt'
  );
  assert.ok(
    newsPlaceholders.every((a) => a.cover_image.startsWith('https://picsum.photos/')),
    'every seeded news story should have a placeholder cover picture'
  );
});

await test('seeding again does not duplicate the placeholders', async () => {
  const before = await db.listArticles({ limit: 50 });
  await db.ensureSchema();
  const after = await db.listArticles({ limit: 50 });
